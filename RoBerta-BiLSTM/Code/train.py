...
def train_and_evaluate_model_with_cv(data, labels):
    ...
    #     # Extract roberta and lstm parameters
    #     roberta_params = [p for p in model.roberta.parameters()]
    #     lstm_params = [p for p in model.lstm.parameters()]
    #     optimizer = optim.AdamW([
    #         {'params': roberta_params, 'lr': 2e-5},  # learning rate for roberta
    ...
    #     print(f"Best Validation Sensitivity: {best_sn:.4f} | Best Validation Specificity: {best_sp:.4f} | "
    ...
    for epoch in range(num_epochs):
        ...
        epoch_model_path = f"../Result/robertabilstm_model_{epoch+1}.pth"
