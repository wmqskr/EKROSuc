...
class Classifier(nn.Module):
    def __init__(self, hidden_size=256, dropout=0.1):
        ...
        # Bidirectional LSTM
        self.lstm = nn.LSTM(input_size=768,  # Input dimension: hidden size of roberta
    ...
    def forward(self, input_ids, attention_mask, flag=0):
        ...
        # last_hidden_state of roberta, shape: (batch_size, seq_len, hidden_size)
        outputs = outputs.hidden_states[12]
        # print(outputs.shape)
        
        # Pass the output from roberta to LSTM layer
        lstm_output, _ = self.lstm(outputs)  # lstm_output shape: (batch_size, seq_len, hidden_size * 2)
        
        # Pooling the output of LSTM (mean pooling is used here)
        # pooled_output = lstm_output.mean(dim=1)  # Mean on the seq_len dimension, output shape: (batch_size, hidden_size * 2)
        lstm_output = lstm_output.permute(0, 2, 1)  # Permute, output shape: (batch_size, hidden_size * 2, seq_len)
        if flag == 1:
